...
// 根据线加速度的波动幅度判断IMU的运动激励是否足够
// 利用relativePose ()找到参考帧l（即世界坐标系的原点）
// 利用sfm.construct()进行 Structure From Motion操作，估计窗口内每一帧的位姿和特征点3D点位置
// 对于所有的图像帧all_image_frame，包括不在滑动窗口中的，估计每一帧的位姿和特征点3D点位置
// 进行视觉惯性联合初始化visualInitialAlign ()
bool Estimator::initialStructure()
{
    TicToc t_sfm;
    // check imu observibility
    //  首先，计算IMU数据的变化。通过计算IMU数据的变化，
    //  以及计算其方差，来评估IMU数据的可观性。如果IMU数据的方差小于阈值0.25，表示IMU数据的激励不足
    {
        map<double, ImageFrame>::iterator frame_it;
        Vector3d sum_g;
        // frame_it++,没有算上all_image_frame的第一帧
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            // make_pair: 特征点被观测到的帧的序号+该帧上对应的归一化平面坐标
            // 一个特征点被N帧图像观测到，就会有N个make_pair
            // tmp_feature是这N个make_pair的集合
            tmp_feature.observation.push_back(make_pair(imu_j, Eigen::Vector2d{pts_j.x(), pts_j.y()}));
        }
        // 特征点的个数是M，则sfm_f是M个tmp_feature的集合
        sfm_f.push_back(tmp_feature);
    }
    Matrix3d relative_R;
    Vector3d relative_T;
    int l;
    // relativePose ()检查从第二帧开始，哪一帧相对于当前帧同时满足下面三点，返回帧号l(字母L)
    // 两帧同时观测到的特征点的数目超过20个
    // 平均视差*460>30
    // 在对极几何中，起作用的特征点的数目大于12
    ...
    GlobalSFM sfm;
    // Q T存储的是从每一帧相机坐标系到第一帧相机坐标系的旋转矩阵和平移矩阵
    // sfm_tracked_points存储的是求解出来在第一帧相机坐标系下所有特征点的3D坐标
    ...
    // 对所有图像帧，包括不在滑动窗口中的，估计每一帧的位姿和特征点的3D位置
